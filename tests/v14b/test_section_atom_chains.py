...
def test_assemble_chains_for_section_can_create_full_typed_chain():
    chain = assemble_chains_for_section(
        [
            _atom(1, "constraint", "Wafer-scale fabrication tolerance is the root constraint."),
...
def test_build_section_atom_chains_from_sqlite(tmp_path):
    ...
    conn = sqlite3.connect(str(db))
    row = conn.execute(
        "SELECT typed_chain_completeness, evidence_grade, claim_scope FROM section_atom_chains"
    ).fetchone()
    conn.close()

    assert stats["chains_written"] == 1
    assert stats["by_completeness"]["full"] == 1
    assert row == ("full", "typed_section_lineage", "bottleneck_lineage_evidence")
