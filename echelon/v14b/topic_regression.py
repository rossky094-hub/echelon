...
def _join_gap_values(values: Any) -> str:
    if values is None:
        return ""
    if isinstance(values, str):
        return values
    if isinstance(values, (list, tuple, set)):
        return ";".join(str(x) for x in values if str(x or "").strip())
    return str(values)


def _repair_plan_priority(priority: Any) -> int:
...
def _repair_plan_candidate_ids(task: dict[str, Any]) -> list[str]:
    out: list[str] = []
    seen: set[str] = set()
    ...
    for pid in task.get("candidate_paper_ids") or []:
        add(pid)
    for paper in task.get("candidate_papers") or []:
        if isinstance(paper, dict):
            add(paper.get("paper_id"))
    for obj in task.get("evidence_objects") or []:
        if isinstance(obj, dict):
            add(obj.get("paper_id"))
            add(obj.get("source_paper_id"))
            add(obj.get("target_paper_id"))
    return out


def _topic_dossier_repair_plan_rows(result: dict[str, Any]) -> list[dict[str, Any]]:
    topic = str(result.get("topic") or "")
    rows: list[dict[str, Any]] = []
    for task in result.get("topic_dossier_repair_plan") or []:
        if not isinstance(task, dict):
            continue
        candidate_ids = _repair_plan_candidate_ids(task)
        rows.append(
            {
                "topic": topic,
                "gap_type": task.get("gap_type") or "topic_dossier_evidence_repair",
                "bottleneck": task.get("bottleneck") or "",
                "priority": _repair_plan_priority(task.get("priority")),
                "candidate_paper_ids": ";".join(candidate_ids),
                "frontfill_query": task.get("frontfill_query") or topic,
                "required_sections": _join_gap_values(task.get("required_sections")),
                "why": task.get("why") or task.get("title") or "Topic Dossier evidence repair target",
                "source_contract": "topic_dossier_evidence_repair_plan",
                "repair_id": task.get("repair_id") or "",
                "target_pipeline_steps": _join_gap_values(task.get("target_pipeline_steps")),
                "retrieval_modes": _join_gap_values(task.get("retrieval_modes")),
                "parser_contract": task.get("parser_contract") or "",
                "claim_scope": task.get("claim_scope") or "evidence_repair_queue_only",
                "evidence_grade": task.get("evidence_grade") or "frontfill_target",
            }
        )
    return rows
...
def build_evidence_gap_rows(result: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    rows.extend(_topic_dossier_repair_plan_rows(result))
    return rows
...
def run_topic_regression(lens: dict[str, Any], benchmark: BenchmarkTopic = METALENS_BENCHMARK) -> dict[str, Any]:
    ...
    result = {
        ...
        "topic_dossier_repair_plan": (lens.get("topic_dossier") or {}).get("evidence_repair_plan") or [],
...
def write_evidence_gap_outputs(results: list[dict[str, Any]], out_dir: Path) -> None:
    ...
    for row in rows[:80]:
        candidate_count = len([x for x in str(row.get("candidate_paper_ids") or "").split(";") if x])
        source = row.get("source_contract") or "topic_regression_gate"
        lines.append(
            f"| {row.get('topic')} | {row.get('gap_type')} | {source} | {row.get('bottleneck') or ''} | "
