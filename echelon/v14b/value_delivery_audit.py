...
def audit_legacy_flow_isolation_contract(repo_root: Path | None = None) -> dict[str, Any]:
    ...
    decision_audit_context = _make_target_context(makefile, "decision-audit", before=0, after=22)
    ...
    return {
        ...
        "policy": (
            ...
            "not raw primary-section presence, and must finish by refreshing the decision-audit loop. The loop must include "
            "a first-principles path-challenge audit and evidence-repair priority report before release readiness so the current route is challenged, not merely executed. "
            "Release readiness must summarize the go/no-go state from current evidence instead of relying on green tests or graph renderability. "
...
def audit_multi_topic_regression(
    ...
    if repo_root is not None:
        ...
        topic_regression_exports_topic_dossier_repair_plan = _source_contains(
            topic_regression_source,
            (
