...
- generated_at: `2026-05-31T02:39:57Z`
...
- section evidence: 5,484 rows / 3,021 papers
- primary section evidence: 3,021 papers (5.5%)
- primary section provenance: 1,131 strong/moderate papers; weak-only=62.6%
- current section parser contract: 376 papers (12.4%)
- section parser contracts: legacy_unknown_contract:4,626, v14b_section_parser_contract_v3_toc_guard:858
- multi-topic evidence-gap queue: 2 / 31 decision-grade section covered (6.5%); raw primary=6 (19.4%)
- topic-gap section triage: `fail`; current-parser no-target=22; stale-contract=6; unattempted-PDF=3
...
- section frontfill health: running_or_unknown [section_delta] (done=394/8592, no_evidence_delta=0, no_evidence_hours=0.0, current_contract_primary=368, contract_status=running_or_unknown, no_current_contract_delta=0, no_current_contract_hours=0.0)
...
- **section_evidence** (high): primary section evidence covers only 3,021 papers. Next: Finish top12000 section ingest, then run delta section queue for main/future/branch/keystone papers.
- **section_evidence_provenance** (medium): primary section evidence quality is still fragile: 1,131 papers have strong/moderate parser provenance; weak-only rate is 62.6%. Next: Use explicit/embedded heading evidence for bottleneck and Claim Card promotion; keep loose/legacy section matches as weak evidence until manually audited or re-parsed.
- **section_parser_contract_coverage** (medium): primary section evidence has current parser-contract coverage for only 376/3,021 papers (12.4%); legacy parser-contract sections may predate TOC/fragment guards. Next: Re-run section evidence with the current parser contract before promoting section-derived bottleneck, Topic Dossier, or Claim Card claims.
- **multi_topic_evidence_gap** (high): multi-topic regression still has decision-grade section evidence for only 2/31 queued benchmark-topic papers (6.5%); raw primary-section coverage is 6/31 (19.4%). Triage: current-parser no-target=22, stale-contract=6, unattempted-PDF=3. No-target inspection: parser-target-signal=0, subthreshold-target-signal=2, sectionless/non-target-heading=11. Next: Do not loosen the current parser for the no-target bucket; keep those papers as weak full-text or metadata evidence and focus repair effort on stale-contract reparse and unattempted PDFs.
