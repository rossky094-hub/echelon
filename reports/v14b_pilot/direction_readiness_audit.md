...
- OpenAlex frontfill health: cooling_down_or_stopped [openalex_backfill_current] (processed=3000/22643, ok=2898, fail=102, cooldown_hours=2.4)
- section evidence: 5,259 rows / 3,002 papers
- primary section evidence: 3,002 papers (5.4%)
- primary section provenance: 765 strong/moderate papers; weak-only=74.5%
- current section parser contract: 0 papers (0.0%)
- section parser contracts: legacy_unknown_contract:5,259
- multi-topic evidence-gap queue: 14 / 55 primary-section covered (25.5%)
- section frontfill health: running_or_unknown [section_delta] (done=1000/6603, no_evidence_delta=0, no_evidence_hours=0.0, current_contract_primary=unknown, contract_status=running_or_unknown, no_current_contract_delta=0, no_current_contract_hours=0.0)
...
- **section_evidence** (high): primary section evidence covers only 3,002 papers. Next: Finish top12000 section ingest, then run delta section queue for main/future/branch/keystone papers.
- **section_evidence_provenance** (medium): primary section evidence quality is still fragile: 765 papers have strong/moderate parser provenance; weak-only rate is 74.5%. Next: Use explicit/embedded heading evidence for bottleneck and Claim Card promotion; keep loose/legacy section matches as weak evidence until manually audited or re-parsed.
- **section_parser_contract_coverage** (medium): primary section evidence has current parser-contract coverage for only 0/3,002 papers (0.0%); legacy parser-contract sections may predate TOC/fragment guards. Next: Re-run section evidence with the current parser contract before promoting section-derived bottleneck, Topic Dossier, or Claim Card claims.
- **multi_topic_evidence_gap** (high): multi-topic regression still has primary section evidence for 14/55 queued benchmark-topic papers (25.5%). Next: After the active top12000 ingest finishes, run make topic-gap-repair to refresh regression gaps, rebuild the topic-gap section queue, ingest targeted papers, and re-audit before promoting Topic Dossier, bottleneck lineage, or Claim Card conclusions.
- **multi_topic_section_provenance** (medium): benchmark-topic gap papers have primary sections, but too many are weak parser matches (71.4% weak-only). Next: Do not pass multi-topic Dossier claims on loose parser evidence alone; re-parse or audit those topic papers before promotion.
...
- **openalex_frontfill_health** (medium): OpenAlex frontfill is cooling_down_or_stopped; processed=3000/22643, cooldown_remaining_hours=2.4. Next: Respect the OpenAlex 429 cooldown; resume conservative backfill after cooldown before promoting cross-field/topic claims.
