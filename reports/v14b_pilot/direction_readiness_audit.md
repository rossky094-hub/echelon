...
- generated_at: `2026-05-31T09:57:45Z`
...
- section evidence: 5,620 rows / 3,030 papers
- primary section evidence: 3,030 papers (5.5%)
- primary section provenance: 1,348 strong/moderate papers; weak-only=55.5%
- current section parser contract: 628 papers (20.7%)
- section parser contracts: legacy_unknown_contract:4,205, v14b_section_parser_contract_v3_toc_guard:1,415
- multi-topic evidence-gap queue: 43 / 78 decision-grade section covered (55.1%); raw primary=48 (61.5%)
- topic-gap section triage: `fail`; current-parser no-target=17; stale-contract=4; unattempted-PDF=8; repair-contract-closed=17/215
- topic-gap no-target inspection: `pass`; parser-target-signal=0; subthreshold-target-signal=1; sectionless/non-target-heading=10
- section frontfill health: running_or_unknown [section_delta] (done=428/8373, no_evidence_delta=0, no_evidence_hours=0.0, current_contract_primary=627, contract_status=running_or_unknown, no_current_contract_delta=0, no_current_contract_hours=0.0)
- raw PDF store: `pass`; success=2,679; probable_pdf_rate=100.0%; section_cache_papers=6; topic_gap_local_pdf=7/38
- topic-gap raw PDF parser dry run: `pass`; local=14; primary-ready=4; repair-ready=2; no-target=6; parser-exception=0; no-target-shapes=sectionless_or_non_target_heading_format:5, heading_like_but_not_target_section:1; actions=weak_fulltext_or_metadata_only:5, weak_primary_context_only:4, already_covered_parser_control:2
...
- **section_evidence_provenance** (medium): primary section evidence quality is still fragile: 1,348 papers have strong/moderate parser provenance; weak-only rate is 55.5%. Next: Use explicit/embedded heading evidence for bottleneck and Claim Card promotion; keep loose/legacy section matches as weak evidence until manually audited or re-parsed.
- **section_parser_contract_coverage** (medium): primary section evidence has current parser-contract coverage for only 628/3,030 papers (20.7%); legacy parser-contract sections may predate TOC/fragment guards. Next: Re-run section evidence with the current parser contract before promoting section-derived bottleneck, Topic Dossier, or Claim Card claims.
- **multi_topic_evidence_gap** (high): multi-topic regression still has decision-grade section evidence for only 43/78 queued benchmark-topic papers (55.1%); raw primary-section coverage is 48/78 (61.5%). Triage: current-parser no-target=17, stale-contract=4, unattempted-PDF=8. Typed-chain triage: atoms-missing=0, chains-missing=1, full-chain-missing=1, topic-mismatch=0. Repair-contract closure: closed=17/215 (7.9%). No-target inspection: parser-target-signal=0, subthreshold-target-signal=1, sectionless/non-target-heading=10. Next: Do not loosen the current parser for the no-target bucket; keep those papers as weak full-text or metadata evidence and focus repair effort on stale-contract reparse and unattempted PDFs.
...
```json
{
  ...
  "created_at": "2026-05-31 08:16:02",
  ...
  "calibration_summary": {
    ...
    "candidate_edges_used": 500,
    "decision_grade_limitation_sections": 50,
